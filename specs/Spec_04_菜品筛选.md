...
- 过敏类目 Concept（kind=allergen）：海鲜（66 个真实标准食材名：基围虾/大闸蟹/三文鱼等）、蟹类（大闸蟹/梭子蟹/螃蟹/蟹肉棒/蟹黄/蟹黄\/蟹膏/青蟹）、坚果（花生/核桃/杏仁等 7 项）、蛋类（4 项）、奶类（4 项）、豆类（4 项）、麸质（3 项）；成员为 Ingredient 标准名。“蟹味菇”不属于蟹类；名字含"海鲜"但不是海鲜食材的（海鲜酱/海鲜菇/海鲜捞汁）不建 is_a 边。
